...
public class SearchDialogAdapter<T extends Searchable>
	...
	private void initializeViews(
		...
		Typeface tf = Typeface.createFromAsset(mContext.getAssets(), "TitilliumWeb-Regular.ttf");
		text.setTypeface(tf);
		text.setTextColor(getColor(R.color.searchDialogResultColor));
		if (mSearchTag != null && mHighlightPartsInCommon) {
			text.setText(StringsHelper.highlightLCS(object.getVNumber(), getSearchTag(),
				getColor(R.color.searchDialogResultHighlightColor)
			));
		} else {
			text.setText(object.getVNumber());
		}
