...
public class SimpleSearchFilter<T extends Searchable> extends BaseFilter {
	...
	protected Filter.FilterResults performFiltering(CharSequence chars) {
		...
		for (T item : mItems) {
			if (item.getVNumber().toLowerCase().contains(filterSeq)) {
				filteredItems.add(new Pair<>(item, filterSeq.length()));
			} else if (mCheckLCS) {
				int lcsLength = StringsHelper.lcs(item.getVNumber(), filterSeq).length();
				if (lcsLength > item.getVNumber().length() * mAccuracyPercentage) {
